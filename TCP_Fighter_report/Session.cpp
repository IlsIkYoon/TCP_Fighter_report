...
bool CreateNewCharacter(Session* _session) {
	...
	for (int i = -1; i < 2; i++)
	{
		for (int j = -1; j < 2; j++)
		{
			...
			it = Sector[_session->_player->_x / SECTOR_RATIO + i][_session->_player->_y / SECTOR_RATIO + j].begin();

			for (; it != Sector[_session->_player->_x / SECTOR_RATIO + i][_session->_player->_y / SECTOR_RATIO + j].end(); it++)
			{
